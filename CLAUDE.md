...
班網 `class-website` 用的是同一招，但**那邊有腳本代勞**：改完 `assets/` 跑 `node scripts/bump-assets.mjs`
（2026-09-09 補上；在那之前班網 16 頁其實一個版本號都沒有，這一行曾是錯的）。
本 repo 目前仍是手改，改 JS／CSS 就把五個 html 的 `?v=` 一起往前推。
